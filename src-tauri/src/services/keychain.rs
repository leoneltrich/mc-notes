...
impl KeychainService {
    pub async fn get_access_token() -> Result<Option<String>, String> {
        ...
        let master_key_raw = match Self::get_master_key() {
            Ok(key) => key,
            Err(e) => return Err(format!("Master Key not found: {}. Please ensure ServeMe Main App is initialized.", e)),
        };
        ...
        let master_key_bytes = if let Ok(bytes) = BASE64_STANDARD.decode(&master_key_raw) {
            if bytes.len() == 32 { bytes } else { 
                return Err(format!("Invalid decoded Base64 key length: expected 32, got {}", bytes.len()));
            }
        } else if let Ok(bytes) = hex::decode(&master_key_raw) {
             if bytes.len() == 32 { bytes } else { 
                return Err(format!("Invalid decoded Hex key length: expected 32, got {}", bytes.len()));
            }
        } else if master_key_raw.len() == 32 {
            master_key_raw.as_bytes().to_vec()
        } else {
            return Err("Master Key must be 32 bytes (raw), 44 bytes (base64), or 64 chars (hex).".into());
        };
        ...
        let resp = match client.get(TOKEN_URL).send().await {
            Ok(r) => r,
            Err(e) => return Err(format!("Could not reach Token Agent: {}. Make sure ServeMe Main App is running.", e)),
        };

        if resp.status().as_u16() == 404 {
            return Err("Token Agent returned 404. Endpoint not found.".into());
        }

        let token_data: TokenResponse = resp.json().await
            .map_err(|e| format!("Failed to parse response from Token Agent: {}", e))?;

        if let Some(err_msg) = token_data.error {
            return Err(format!("Token Agent error: {}", err_msg));
        }

        let ciphertext_b64 = token_data.ciphertext
            .ok_or_else(|| "Response missing ciphertext".to_string())?;
        let nonce_b64 = token_data.nonce
            .ok_or_else(|| "Response missing nonce".to_string())?;
    ...
    fn get_master_key() -> Result<String, String> {
        ...
        entry.get_password().map_err(|e| format!("Failed to read Master Key: {}", e))
    }
    ...
    fn decrypt_token(key_bytes: &[u8], ciphertext_b64: &str, nonce_b64: &str) -> Result<Option<String>, String> {
        ...
        let nonce_vec = BASE64_STANDARD.decode(nonce_b64)
            .map_err(|e| format!("Failed to decode nonce Base64: {}", e))?;
        
        if nonce_vec.len() != 12 {
            return Err(format!("Invalid nonce length: expected 12 bytes, got {}", nonce_vec.len()));
        }

        let nonce = Nonce::from_slice(&nonce_vec);

        let plaintext_bytes = cipher.decrypt(nonce, ciphertext.as_ref())
            .map_err(|e| format!("Decryption error: {}. Check if the Master Key matches the encryption key.", e))?;
